...
/* Ставит волка обязательно первым, а капусту обязательно последней */
...
/* Новый список Xs без первого элемента X */
select(X, [X|Xs], Xs). 

/* Рекурсивная функция удаления элемента из списка. Сначала удаляет все первые. При нахождении нужного удаляет его, а потом добавляет те, которые удалила. */
